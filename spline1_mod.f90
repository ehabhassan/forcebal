...
!    1996, p.251
!  W.A.Houlberg, P.I.Strand, D.C.McCune 8/2001
...
!  D.C.McCune, corrected BCs for options k_bc=6,7
!  W.A.Houlberg, F90 free form 8/2004
!
!Comments:
...
REAL(KIND=rspec) :: &
  a1,an,b1,bn,el21,elnn1,hn,q,t,wk(n)
...
SUBROUTINE SPLINE1_INTEG(k,n0,x0,f,n1,x1, &
                         value,iflag,message)
!-------------------------------------------------------------------------------
!SPLINE1_INTEG evaluates the integral f(x)*x**k, where f(x) is a cubic
!  spline function and k >= 0
!
!References:
!  Forsythe, Malcolm, Moler, Computer Methods for Mathematical
...
!  W.A.Houlberg, P.I.Strand, D.McCune 8/2001
!  W.A.Houlberg, F90 free form 8/2004
!  W.A.Houlberg, generalized to arbitrary k 8/2006
!-------------------------------------------------------------------------------

!Declaration of input variables
INTEGER, INTENT(IN) :: &
  k,                   & !exponent in integral (s(x)*x**k) [-]
                         !>=0
...
IF(n1 <= 0) THEN
  ...
  iflag=-1
  message='SPLINE1_INTEG(1)/WARNING:no target points'
  GOTO 9999
...
DO i=1,n1 !Over target abscissas
  ...
  DO WHILE(ido == 0) !Over source abscissas
    ...
    !Integrate over dx
    add= (xnew**(k+1)-xold**(k+1))/(k+1) &
        *(f(1,j)-xold*f(2,j)+xold**2*f(3,j)/2-xold**3*f(4,j)/6) &
        +(xnew**(k+2)-xold**(k+2))/(k+2) &
        *(f(2,j)-xold*f(3,j)+xold**2*f(4,j)/2) &
        +(xnew**(k+3)-xold**(k+3))/(k+3) &
        *(f(3,j)-xold*f(4,j))/2 &
        +(xnew**(k+4)-xold**(k+4))/(k+4) &
        *f(4,j)/6
    ...
    IF(j > n0) THEN
      ...
      iflag=1
      message='SPLINE1_INTEG(2)/ERROR:target node out of range'
      GOTO 9999
